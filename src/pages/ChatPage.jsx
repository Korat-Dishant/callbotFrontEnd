...
const ChatPage = () => {
  ...
  const user_email = user?.user?.primaryEmailAddress?.emailAddress;
  const date = `${user?.user?.lastSignInAt?.getDate()}/${
    user?.user?.lastSignInAt?.getMonth() + 1
  }`;
  useEffect(() => {
  ...
  const onSubmit = async (event) => {
    ...
    setLoading(true);
    const response = await ask(message, user_name, user_email, date);
    setTimeout(() => {
      if (response.success) {
